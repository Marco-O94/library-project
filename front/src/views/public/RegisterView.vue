    ...
    <div class="px-6 h-full">
      <div class="flex justify-center items-center flex-wrap h-full g-6 text-gray-800">
        <div class="md:w-8/12 lg:w-6/12 mb-12 md:mb-0">
          <img src="@/assets/images/loginImage.png" class="w-full" alt="Phone image" />
        </div>
        <div class="w-10/12 lg:w-5/12 lg:ml-20">
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4 align-middle items-center mb-8">
            <div class="link-grow inline-block col-span-1">
              <router-link :to="{ name: 'login' }"><svg class="inline-block mr-3 w-5 h-5"
          ...
          <form @submit.prevent="userStore.registerRequest(registerData)">
            ...
            <div class="mb-6">
              <input v-model="registerData.password_confirmation" type="password"
                class="form-control block w-full px-4 py-2 text-xl font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
                placeholder="Conferma la password" />
            </div>
            <ErrorField v-if="generalStore.errors.password_confirmation"
              :errors="generalStore.errors.password_confirmation" />


            <!-- Submit button -->
            <button
              v-if="(registerData.name && registerData.email && registerData.password && registerData.password_confirmation && registerData.password === registerData.password_confirmation)"
              type="submit"
