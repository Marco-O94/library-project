    ...
    <section class="py-12 bg-white sm:py-16 lg:py-20">
        <div class="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
            ...
            <template v-else>
                    ...
                    <div class="relative group" v-for="book, index in bookStore.books.data" :key="index">
                        <div class="overflow-hidden aspect-w-1 aspect-h-1">
                            ...
                            <div class="absolute left-3 top-3 flex flex-nowrap gap-2">
                                <p v-for="cat, index in book.categories" :key="index"
                                    :class="'bg-' + cat.color + '-200 bg-' + cat.color + '-300'"
                                    class="sm:px-3 sm:py-1.5 px-1.5 py-1 text-[8px] sm:text-xs text-white font-bold tracking-wide uppercase rounded-full">
                                    {{cat.name}}</p>
                            </div>
                        </div>
                ...
                <ListPagination scope="publicBooks" :currentPage="bookStore.books.current_page"
