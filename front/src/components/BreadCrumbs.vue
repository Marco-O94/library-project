...
const paths = computed(() => path.value.split('/'));
...
<template>
    <nav class="flex" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
          ...
          <li v-if="(index > 0 && index <= paths.length - 2)">
            <router-link :to="{name: route}" class="flex items-center">
              ...
              <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">{{route}}</span>
            </router-link>
          </li>
          <li v-else-if="index === paths.length - 1">
            <a @click="$router.go(-1)" class="flex items-center">
              <svg v-if="index !== paths.length - 1" class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
              <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">{{route}}</span>
            </a>
          </li>
