    ...
    <div class="flex justify-center">
        <nav aria-label="Page navigation">
            <ul class="flex list-style-none">
                <!-- Previous -->
                <li v-if="prevPage" @click="getPage(props.currentPage ? props.currentPage - 1 : 0)" class="page-item cursor-pointer"><span
                ...
                <template v-for="index in props.lastPage" :key="index">
                    <li @click="getPage(index)" v-if="props.currentPage === index" class="page-item active"><span
                            class="page-link cursor-default relative block py-1.5 px-3 rounded border-0 bg-blue-600 outline-none transition-all duration-300  text-white hover:text-white hover:bg-blue-600 shadow-md focus:shadow-md">{{ index }}
                            <span class="visually-hidden">(current)</span></span></li>
                    <li @click="getPage(index)" v-else class="page-item"><span
                            class="page-link cursor-pointer relative block py-1.5 px-3 rounded border-0 bg-transparent outline-none transition-all duration-300  text-gray-800 hover:text-gray-800 hover:bg-gray-200 focus:shadow-none">{{ index }}</span>
                    </li>

                </template>
                <!-- Next -->
                <li v-if="nextPage" @click="getPage(props.currentPage ? props.currentPage + 1 : 0)" class="page-item cursor-pointer"><span
